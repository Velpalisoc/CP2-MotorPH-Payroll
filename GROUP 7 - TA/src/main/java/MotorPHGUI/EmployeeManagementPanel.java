...
public class EmployeeManagementPanel extends JPanel {
    ...
    private void handleSearchChange() {
        ...
        if (searchText.isEmpty() && originalModel != null) {
            ...
            newModel = copyTableModel(originalModel);
        } else {
            // Create filtered model with same columns as original
            newModel = new DefaultTableModel(columns, 0) {
                @Override
                public boolean isCellEditable(int row, int column) {
            ...
            for (int row = 0; row < originalModel.getRowCount(); row++) {
                boolean matchFound = false;
                for (int col = 0; col < originalModel.getColumnCount()-1; col++) { // Skip Actions column
                    String cellValue = originalModel.getValueAt(row, col).toString().toLowerCase();
        ...
        employeeTable.setModel(newModel);
        // Reapply button renderer and editor
        employeeTable.getColumn("Actions").setCellRenderer(new ActionCellRenderer());
        employeeTable.getColumn("Actions").setCellEditor(new ActionCellEditor(this));
    }
    ...
    public void handleDelete(int row) {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            for (int i = 0; i < 6; i++) {
                employeeData[i] = model.getValueAt(row, i).toString();
            }

            // Log activity
            logActivity("Admin", "Delete", "", employeeData);

            // Remove from table and update CSV
            model.removeRow(row);
            syncTableToCSV();
            ...
            // Create a new table model with the current data
            ...
            // Reapply the custom renderer and editor for Actions column
            employeeTable.getColumn("Actions").setCellRenderer(new ActionCellRenderer());
            employeeTable.getColumn("Actions").setCellEditor(new ActionCellEditor(this));
    ...
    private void logActivity(String user, String action, String details, String[] employeeData) {
        ...
        // Build CSV line without any extra text
