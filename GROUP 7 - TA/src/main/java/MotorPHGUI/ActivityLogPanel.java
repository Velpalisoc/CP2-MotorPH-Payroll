...
public class ActivityLogPanel extends JPanel {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(logFile))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length >= 7) {
                    ...
                    // Fix alignment by directly using array indices
                    row[4] = parts[4];  // ID no.
                    row[5] = parts[5];  // Last Name
                    row[6] = parts[6];  // First Name
                    row[7] = "Restore"; // Restore button
            ...
            JOptionPane.showMessageDialog(this, 
                "Error loading activity logs: " + ex.getMessage(),
                "Error", 
    ...
    public void handleRestore(int row) {
        ...
        // For "Delete" actions, reconstruct employee data in correct order
        if (action.equals("Delete") && parts.length >= 10) {
            // Map the data to match CSV structure
            // Format should be: ID,LastName,FirstName,PhoneNumber,Status,Position
            String[] employeeData = new String[6];
            employeeData[0] = parts[4]; // ID no.
            employeeData[1] = parts[5]; // Last Name
            employeeData[2] = parts[6]; // First Name
            employeeData[3] = parts[7]; // Phone Number
            employeeData[4] = parts[8]; // Status
            employeeData[5] = parts[9]; // Position
            ...
            JOptionPane.showMessageDialog(this, "Restore is only supported for deleted employees with full data.", 
